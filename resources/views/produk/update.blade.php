...
<form method="POST" action="{{ route('produk.update', ['id' => $model->id]) }}">
            ...
            <div class="col-md-6">
                <select id="kategori_id" name="kategori_id" class="form-control @error('kategori_id') is-invalid @enderror">
                    ...
                    <option value="{{$value->id}}" 
                        @if(old('kategori_id', $model->id) == $value->id)
                            selected="selected"
                        @endif
                    >{{$value->nama}}</option>
                    @endforeach
    ...
    <div class="form-group row">
        <label for="harga" class="col-md-4 col-form-label text-md-right">{{ __('harga') }}</label>
            ...
            <div class="col-md-6">
                <input id="harga" type="text" class="form-control @error('harga') is-invalid @enderror" name="harga" value="{{ old('harga',$model->harga) }}"R>
    ...
    <div class="form-group row">
        <label for="diskon" class="col-md-4 col-form-label text-md-right">{{ __('diskon') }}</label>
            ...
            <div class="col-md-6">
                <input id="diskon" type="text" class="form-control @error('diskon') is-invalid @enderror" name="diskon" value="{{ old('diskon', $model->diskon) }}"R>
    ...
    <div class="form-group row">
        <label for="deskripsi" class="col-md-4 col-form-label text-md-right">{{ __('Deskripsi') }}</label>
            ...
            <div class="col-md-6">
                <input id="deskripsi" type="text" class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" value="{{ old('deskripsi', $model->deskripsi) }}">
